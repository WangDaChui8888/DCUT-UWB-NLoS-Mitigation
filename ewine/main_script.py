...
def main():
    ...
    parser.add_argument('--dt_batch_size', type=int)
    ...
    if args.transformer_layers is not None: config.DUAL_TRANSFORMER_MODEL_PARAMS['transformer_layers'] = args.transformer_layers
    if args.attention_heads is not None:    config.DUAL_TRANSFORMER_MODEL_PARAMS['attention_heads']  = args.attention_heads
    if args.loss_weight_error is not None:  config.DUAL_TRANSFORMER_LOSS_WEIGHTS['error_output'] = args.loss_weight_error
    if args.learning_rate is not None:      config.DUAL_TRANSFORMER_LEARNING_RATE = args.learning_rate
    if args.dropout_rate is not None:       config.DUAL_TRANSFORMER_MODEL_PARAMS['dropout_rate']  = args.dropout_rate
    if args.ff_dim is not None:             config.DUAL_TRANSFORMER_MODEL_PARAMS['ff_dim']        = args.ff_dim
    if args.dt_batch_size is not None:      config.DUAL_TRANSFORMER_BATCH_SIZE = args.dt_batch_size
    if args.dnn_lr is not None:             config.DNN_LEARNING_RATE = args.dnn_lr
    if args.dnn_dropout is not None:        config.DNN_MODEL_PARAMS['dropout_rate'] = args.dnn_dropout
    if args.svm_c is not None:              config.SVC_PARAMS['C'] = args.svm_c
    if args.ridge_alpha is not None:        config.RIDGE_PARAMS['alpha'] = args.ridge_alpha
    ...
    if not all_dataframes:
        logging.error("[Fatal] No valid data loaded. Check 'RELATIVE_DATA_PATHS'."); return
    ...
    if data is None or data.empty:
        logging.error("[Fatal] Data empty after cleaning."); return

    target_nlos_numeric = map_labels_and_check(data['NLOS'])
    if target_nlos_numeric is None:
        logging.error("[Fatal] Label mapping failed."); return
    ...
    if split_results is None:
        logging.error("[Fatal] Split failed."); return
